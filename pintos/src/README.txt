...
This struct will contain information required for a priority to be donated to the previous lower priority thread. It will also hold a semaphore that will later be initialized to 0 to wait for the lock to be released.
...
The list_insert_ordered function of list ensured the highest priority thread was placed at the front of the list. 

>> B4: Describe the sequence of events when a call to lock_acquire()
>> causes a priority donation.  How is nested donation handled?

When the lock_acquire causes a priority donation, it will first check if the current thread owns the lock or not. If the current thread does not own the lock, it will call the function thread_donate_priority (to-be implemented). In this function, current thread, thread needing donation and the priority of the donee thread will be saved into the struct described in B1. Then the semaphore will be initialized to 0 and sema_down will be executed to wait for the lock to be released.

>> B5: Describe the sequence of events when lock_release() is called
>> on a lock that a higher-priority thread is waiting for.

When the lock_release is called, the semaphore will be incremented through sema_up and the lock will proceed to release itself. Then thread_callback (to-be implemented) function will be called to retrieve the very first item of the priority-based sorted list to continue executing the threads. 
...
If two threads try to set priority at the same time, the race condition could arise. We are still working on this part to avoid such situation.
...
This designed seemed the most logically possible.
