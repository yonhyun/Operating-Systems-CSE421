...
thread_unblock (struct thread *t) 
{
  ...
  /**
   * this can be used for future implementation
   * whenever thread is created it put in on the readylist by priority
   */
...
/*yield thread for a thread if priority in the list has higher priority*/
...
thread_set_priority (int new_priority) 
{
  ...
  thread_set_priority_2(thread_current(),new_priority,true);

}
void thread_set_priority_2(struct thread *cur_thread,int new_priority,bool restore)
{
  /**
   * if the thread is donated one,
   *//////////////////////////////the point 
if (!cur_thread->donated)
{   /*not donated, then just get the new_priority*/
    cur_thread->priority = new_priority;
    cur_thread->original_priority = new_priority;
  /* since it is not the donated one we just replace it all */
}
else if (restore){
...
else{cur_thread->priority=new_priority;}

if (cur_thread->status == THREAD_RUNNING && !list_empty(&ready_list)) 
{ 
  /**
   * ready list is already sorted so, in max_priority the variable is thread_curretn()
   */
  max_priority();
}
